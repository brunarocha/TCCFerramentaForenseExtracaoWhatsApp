...
public class TelaNovoCaso extends javax.swing.JDialog {
    ...
    private void preencheDataCriacao(){
        labelDataCriacao.setText(ConversorData.getStringDataAtual());
    }
    ...
    private void preencheObjetoCaso(){
        novoCaso = new Caso(campoDescricao.getText(),
                            campoNomeInvestigado.getText());
        novoCaso.setUsuario(usuario);
        novoCaso.setUsuariosAcesso(usuariosSelecionados);
    }
    
    private boolean criaPastaCaso(){
        File arquivoNovo = new File(usuario.getDiretorioCasos()+"\\"+novoCaso.getDescricao());
        arquivoNovo.mkdir();
        ...
        if(casoDAO.verificaNomeCaso(campoDescricao.getText())){
            ...
            if(criaPastaCaso()){
                if(casoDAO.inserir(novoCaso)){
                    casoDAO.relacionaCasoUsuario(TipoAcesso.Criador, usuario.getId(), 0L); // usuario criador
                    
                    if(!usuariosSelecionados.isEmpty()){
                        for(Usuario usuarioSelecionado : usuariosSelecionados){
                            casoDAO.relacionaCasoUsuario(TipoAcesso.Visualizar, usuarioSelecionado.getId(), 0L);
                        }
                    }
