...
import model.list.JListModel;
import model.Consulta;
import model.Contato;
import model.Dispositivo;
import model.Mensagem;
import model.gerenciatela.GerenciaTelaDispositivo;
import model.table.TabelaModeloMensagem;
 ...
 * @author BRUNA
 */
public class TelaPrincipalDispositivo extends javax.swing.JFrame {
    ...
    private void preenchePainelInformacao(){
        ...
        labelModelo.setText(dispositivo.getNumeroModelo()+" "+dispositivo.getModelo());
        labelVersaoAndroid.setText(dispositivo.getVersaoAndroid());
    ...
    private void jListContatosMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jListContatosMouseClicked
        if(evt.getClickCount() > 1 && !contatos.isEmpty()){
            carregaMensagemContatoSelecionado(contatos.get(jListContatos.getSelectedIndex()));
    ...
    private void jListGruposMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jListGruposMouseClicked
        if(evt.getClickCount() > 1 && !contatos.isEmpty()){
            carregaMensagemContatoSelecionado(grupos.get(jListGrupos.getSelectedIndex()));
            jListContatos.clearSelection();
        }
    }//GEN-LAST:event_jListGruposMouseClicked

    public void carregaMensagemContatoSelecionado(Contato contato){
    ...
    private void botaoPesquisarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoPesquisarActionPerformed
        ...
        consulta.carregaObjetoConsulta(campoPesquisaPalavra.getText(), campoPesquisaDataInicial.getValue(),
                                       campoPesquisaDataFinal.getValue(), campoPesquisaHoraInicial.getValue(),
                                       campoPesquisaHoraFinal.getValue(), checkContatos.isSelected(), 
                                       checkGrupos.isSelected(), contatoSelecionado);
        ...
        if(!checkContatos.isSelected() && !checkGrupos.isSelected() && consulta.getContato() == null){
            ...
            if(consulta.verificaPreenchimentoCampos()){
                getContentPane().setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                MensagemDAO mensagemDAO = new MensagemDAO(this.dispositivo.getTabelaMensagem(), this.dispositivo.getTabelaContato());
    ...
    private void preencheJListContatos(int indice){
        ContatoDAO contatoDAO = new ContatoDAO(dispositivo.getTabelaContato());
    ...
    private void preencheTotalMensagens(){
        ...
        for(int i = 0 ; i <mensagens.size() ; i++){
            if(mensagens.get(i).isKeyFromMe()){
                contEnviadas++; 
            } else{ 
                contRecebidas++; 
            }
        ...
        labelTotalMensagens.setText(textoTotalMensagem(mensagens.size(), contEnviadas, contRecebidas));
    }
    
    
    private String textoTotalMensagem(int total, int enviadas, int recebidas){
        return "Total: "+total+" | "+"Enviadas: "+enviadas+" | Recebidas: "+recebidas;
    }
...
private class TextAreaCellRenderer extends JTextArea implements TableCellRenderer {  
    ...
    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
        ...
        if(m.isKeyFromMe()){ // se for para mim, deixa o fundo verde
